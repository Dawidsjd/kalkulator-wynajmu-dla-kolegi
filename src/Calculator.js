...
function Calculator(){
  ...
  const [bazowaCena, setBazowaCena] = useState(100.00); // bazowa cena wypożyczenia samochodu na dobę
  ...
  if (kategoriaCenowa == "Premium" && (2023 - lataDoswiadczenia) < 3){
    alert("Niestety nie możesz wypożyczyć samochodu z kategorii Premium, ponieważ posiadasz prawo jazdy krócej niż 3 lata.");
  }
  ...
  useEffect(() => {
    ...
    return (
      <div className="main_container">
        ...
        <div className="cal_container">
          <div className="row1">
            ...
            <Slider defaultValue={50} aria-label="Default" valueLabelDisplay="auto" step={100} marks min={0} max={1000} onChange={handleKilometersChange}/>
        ...
        />
          ...
          <div className="row3">
              <label htmlFor="rental_end">Termin wypożyczenia samochodu (do): </label>
              <input type="date" id="rental_end" name="rental_end" onChange={e => setDataKoncowa(e.target.value)}/><br/>
            </div>

            <div className="row4">
           
              <label htmlFor="rental_start">Termin wypożyczenia samochodu (od): </label>
              <input type="date" id="rental_start" name="rental_start" onChange={e => setDataPoczatkowa(e.target.value)}/><br/>
            </div>
      ...
      {dataPoczatkowa && dataKoncowa && (
        ...
        <p>Cena całkowita netto za {obliczIloscDni()} dni: {cenaCalkowita.toFixed(2)} zł</p>
        <p>Cena całkowita brutto za {obliczIloscDni()} dni: {cenaBrutto.toFixed(2)} zł</p>
